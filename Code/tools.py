...
def plot_affine_zones_with_meshgrid_2D_border_3D(fig, zones_affines, constraints, x_range=(0, 1), y_range=(0, 1), resolution=1000):
    if resolution < 1000:
...
def plot_affine_zones_with_meshgrid_2D_border_neighbours_3D(fig, points_pattern, x_range=(0, 1), y_range=(0, 1), resolution=1000):
    border_points = []
...
def plot_affine_zones_with_meshgrid_2D_border_neighbours(fig, points_pattern, x_range=(0, 1), y_range=(0, 1), resolution=1000):
    border_points = []
    # Créer un meshgrid
    x = np.linspace(x_range[0], x_range[1], resolution)
    y = np.linspace(y_range[0], y_range[1], resolution)
...
def plot_points_in_2D_3D(X_train, fig):
    x = X_train[:, 0]
    y = X_train[:, 1]
    z = np.zeros_like(x)
...
def plot_points_in_2D_2D(X_train, fig):
    x = X_train[:, 0]
    y = X_train[:, 1]
...
def plot_affine_zones_with_meshgrid_2D_border_2D(fig, zones_affines, constraints, x_range=(0, 1), y_range=(0, 1), resolution=1000):
    if resolution < 1000:
        print('Attenion, une résolution inférieur à 1000 peut ne pas fonctionner !')
    # Créer un meshgrid
    x = np.linspace(x_range[0], x_range[1], resolution)
